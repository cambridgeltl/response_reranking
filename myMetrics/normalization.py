...
def normalize_data(utts_list):
    ...
    for utt in utts_list:
        utt = slot_name_normalizer(utt.lower())
        utt = md.detokenize(mt.tokenize(utt.replace('-s', '').replace('-ly', '')))
        ans.append(utt)
    return ans
...
def normalize_slot_name(slot_name):
    ...
    slot_name_mapping = {
     ...
     'PRICE'     : ['price', 'value_price', 'entrancefee', 'entrance fee', 'train_price', 'attraction_entrancefee', 'pricerange', 'value_pricerange', 'price range', 'restaurant_pricerange', 'hotel_pricerange', 'attraction_pricerange', 'attraction_price', "value_ pricerange "],
